...
def _get_document_preview(doc: Document) -> DocumentPreviewResponse:
    ...
    if file_type == "docx":
        parsed = DocxParser.parse(resolved_path)
        content, truncated = _truncate_text(parsed.get("full_text", ""))
        return DocumentPreviewResponse(
            file_name=doc.original_filename,
            file_type=file_type,
    ...
    if file_type == "xlsx":
        parsed = XlsxParser.parse(resolved_path)
...
async def get_onlyoffice_config(
    ...
    public_raw_url = f"{callback_base}/onlyoffice/raw-public/{doc.id}?expires={expires}&token={token}"
