...
import { Search, RefreshCw, Loader2, Network as NetworkIcon, List, Send } from 'lucide-react'
...
export default function KnowledgeGraph() {
  ...
  useEffect(() => {
    if (containerRef.current && viewMode === 'graph' && nodes.length > 0) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass overflow-hidden">
        {viewMode === 'graph' ? (
          <div ref={containerRef} className="h-[500px] w-full bg-slate-900/50">
            {nodes.length === 0 && !isLoading && (
              <div className="h-full flex items-center justify-center">
                <div className="text-center">
                  <NetworkIcon className="w-16 h-16 mx-auto mb-4 text-slate-600" />
                  <p className="text-slate-400">暂无知识图谱数据</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {nodes.map((node) => (
                  <div key={node.id} className="p-4 rounded-xl bg-white/5 border border-white/10">
                    ...
                    {node.value && <p className="text-sm text-slate-400 mt-1">{node.value}</p>}
                  </div>
                ))}
              </div>
            ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="stat-card">
          ...
          <p className="text-sm text-slate-400">实体节点</p>
        </div>
        <div className="stat-card">
          <p className="text-2xl font-bold text-white">{edges.length}</p>
          <p className="text-sm text-slate-400">关系边</p>
        </div>
        <div className="stat-card">
          <p className="text-2xl font-bold text-white">{Array.from(new Set(nodes.map(n => n.type))).length}</p>
          <p className="text-sm text-slate-400">实体类型</p>
        </div>
        <div className="stat-card">
          <p className="text-2xl font-bold text-white">{Array.from(new Set(edges.map(e => e.type))).length}</p>
          <p className="text-sm text-slate-400">关系类型</p>
        </div>
      </div>
    </div>
  )
