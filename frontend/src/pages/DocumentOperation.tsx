...
export default function DocumentOperation() {
  ...
  const openPreview = (action: ActionData) => {
    ...
    if (items.length === 0) {
      toast.error('当前结果暂无可预览的修改内容')
      return
    }

    setPreviewState({
      title: action.title || '文档修改预览',
      description: action.description || '查看本次文档修改的前后差异',
    ...
    <div className="glass relative flex flex-col h-[calc(100vh-200px)]">
                ...
                {message.action.action_type === 'completed' && message.action.result?.preview && (
                  <div className="mt-2 flex">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg border border-blue-400/20 bg-blue-500/10 px-3 py-2 text-sm text-blue-300 transition-colors hover:bg-blue-500/20"
                    >
      ...
      {previewState && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-950/70 p-6 backdrop-blur-sm">
          <div className="max-h-[85vh] w-full max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl">
            <div className="flex items-start justify-between border-b border-white/10 px-6 py-4">
              <div>
                <h3 className="text-lg font-semibold text-white">{previewState.title}</h3>
                <p className="mt-1 text-sm text-slate-400">{previewState.description}</p>
                <p className="mt-2 text-xs text-slate-500">
                  共 {previewState.totalChanges} 处修改
                  {previewState.outputFilename ? ` · ${previewState.outputFilename}` : ''}
                </p>
              </div>
              <button
            ...
            <div className="max-h-[calc(85vh-88px)] space-y-4 overflow-y-auto p-6 scrollbar-thin">
              {previewState.items.map((item, index) => (
                <div
                  key={`${item.op}-${item.paragraph_index}-${index}`}
                  className="rounded-xl border border-white/10 bg-white/5 p-4"
                >
                  <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
                    <span className="rounded-full bg-primary-500/20 px-2.5 py-1 text-primary-300">
                      {item.op}
                    </span>
                    <span className="rounded-full bg-white/10 px-2.5 py-1 text-slate-300">
                      段落 {item.paragraph_index >= 0 ? item.paragraph_index : '-'}
                    </span>
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="rounded-xl border border-red-400/15 bg-red-500/5 p-4">
                      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-red-300">修改前</div>
                      <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-6 text-slate-300">
                        {item.before || '无'}
                      </pre>
                    </div>
                    <div className="rounded-xl border border-green-400/15 bg-green-500/5 p-4">
                      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-green-300">修改后</div>
                      <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-6 text-slate-100">
                        {item.after || '无'}
                      </pre>
                    </div>
                  </div>
